...
use diesel::{self, Connection, SqliteConnection};
use dotenv::dotenv;

use crate::{actions as adapter, models::*};
...
impl Adapter for DieselAdapter {
    async fn load_policy(&self, m: &mut dyn Model) -> Result<()> {
        ...
        #[cfg(feature = "runtime-tokio")]
        let rules = spawn_blocking(move || {
            let conn = conn.lock().unwrap();
            adapter::load_policy(conn.deref())
        })
        .await
    ...
    async fn save_policy(&mut self, m: &mut dyn Model) -> Result<()> {
        ...
        #[cfg(feature = "runtime-tokio")]
        {
            spawn_blocking(move || {
                let conn = conn.lock().unwrap();
                adapter::save_policy(conn.deref(), rules)
            })
            .await
            .map_err(|e| casbin::error::AdapterError(Box::new(e)))?
        }
